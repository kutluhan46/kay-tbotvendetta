...
exports.run = async (client, message, args) => {
 if (!message.member.roles.cache.has("819167989856862230") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new Discord.MessageEmbed().setDescription('Bu komutu kullanabilmek için gerekli yetkiye sahip değilsin!').setColor("Black"));
...
await member.setNickname(`☭ ${isim} | ${yas}`)
  member.roles.add("819097388793724949"); 
  member.roles.remove("819146061406797825");
     const kanal = message.guild.channels.cache.find(c => c.id == "819164522707025930")
  ...
  .setColor("RANDOM")
  .setTimestamp()
  .addField(`Kayıt İşlemi Başarılı`, `<@!${member.id}> **Adlı Kullanıcıya <@&819097388793724949> Rolünü Verdim ve İsmini** \n\`• ☭ ${isim} | ${yas}\` **Olarak Düzenledim**`) 
